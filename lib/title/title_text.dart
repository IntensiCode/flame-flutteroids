...
class TitleText extends Component with HasPaint {
  ...
  static const _text = 'flutteroids';
  ...
  void render(Canvas canvas) {
    ...
    _saved = paint.color;
    _render_history(canvas);
    _render_title(canvas);
    paint.color = _saved;
  }

  void _render_title(Canvas canvas) {
    ...
    _set_color(alpha: 1.0);
    _draw_vector_text(canvas, currentScale, currentStartY);
    paint.maskFilter = _blur;
  }

  void _set_color({double alpha = 1.0}) {
    if (_saved.a < 1.0 || alpha < 1.0) {
      paint.color = Colors.yellow.withValues(alpha: _saved.a * alpha);
    } else {
      paint.color = Colors.yellow;
    }
    _font.tint = paint.color;
  }

  void _render_history(Canvas canvas) {
    paint.maskFilter = _blur;

    final int historyCount = _history.length;
    for (int i = 0; i < historyCount; i += 1) {
      final entry = _history[i];
      final double fraction = (i + 1) / (_historyLength + 1);
      final double ghost = fraction * 0.5;
      _set_color(alpha: ghost);
      _draw_vector_text(canvas, entry.scale, entry.startY);
    }

    paint.maskFilter = null;
  }

  void _draw_vector_text(Canvas canvas, double scale, double startY) {
    if (scale < 0.01) return;
    _offset.dx = game_width / 2;
    _offset.dy = startY;
    _font.scale = scale;
    _font.drawStringAligned(canvas, _offset.dx, _offset.dy, _text, Anchor.topCenter);
  }
}
